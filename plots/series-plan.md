...
## S1(既刊)ブラッシュアップ計画(改稿バッチ・進行中)

全設計図確定(S6〜S10プロット)と認知リズム規範の導入を受けた、S1既刊の見直し。**技術内容・用語割当・クイズは変更しない**。1話ずつ改稿しPR(S1-01から順)。

1. **シーズンアーク名の付与**: S1「取り出せる形」(社是の言葉)。README反映済み。
2. **冒頭ログ断片の統一**: S3以降で定着した冒頭のログ断片装置を、S1全話に適用する(各話の火種を一行の無機質な記録で先置き)。S1-01適用済み。
3. **認知リズム規範の適用パス**: 各話を点検手順(話題テスト/漏出テスト/緊張台帳/拍/境界)にかける。S1-01の主な処置——(a)語り手の未来予告3連(「意味が分かるのはあとになる」「2週間後、痛いほど知る」「5日後に砕ける」)を1つに減らし、他2つは「張り→当事者の回収」へ変換(瀬名の一言は第五章で美咲が回収、城戸の警告は事故現場で耳の奥に鳴る) (b)Multi-Attach棄却の長段落を「思い込み→条件で崩れる」の拍に分割 (c)「ところで——」の話題転換を状況側の入りに。
4. **用語コラム形式のロールアウト**: storage/02〜05とresilience_and_dr/01の用語表を、storage/01と同じコラム形式(太字一行決め手+2〜4文)へ統一する。
5. **接続線の微修正(数行)**: storage/05の風間初出にnet/06(WAF回)への含みを一行。storage/02のAWS Backup節にstorage/06で効く「復元訓練」への含みを一行。gp3移行の夜(S1-02)はS3-03/S4-03から参照されているため描写の整合を確認。
6. **表記統一**: 既刊の「編」番号をシーズン表記(S1-01〜S1-07)に揃える(ファイル名は変更せずfront matterと冒頭表記のみ)。resilience_and_dr/01の位置付け表記を確認。
7. **シードの追加はしない**: 後付けの伏線植え込み(戸倉・置き傘等)は行わない(S1-01の既存シード——青柳の面接の台詞、狩野の「止めてはいけないやつ」、高知の走り書き、守屋の紙——が既に全シリーズと接続しており、追加は過積載)。
