...
| 10 | 波の高さを知る日(完結) | performance_and_scaling + 総集 | 1 | plots/performance-01 | 4年目9月 |
...
## S10 完結編(6語+総集/1話+幕間1本)

**「波の高さを知る日」**: 4年目9月。南海トラフ臨時情報(巨大地震注意)の発表から解除まで——**何も壊れない。壊れる前に備える数日間**を、4年分の設計の答え合わせとして描く完結の中編(7,000〜9,000字)。performance_and_scaling残り6語(auto_scaling, horizontal_scaling, vertical_scaling, stateless_workload, stateful_workload, api_gateway_throttling_usage_plan。transfer_accelerationはS1-01担当済み)は「無意識にやってきたことに正しい名前が付く」答え合わせの言葉として使う。災害原則(universe.md)を厳守。

- **構図「助けに来る人々」**: メインチームの備えの数日に、歴代のゲストが自分の持ち場から一手だけ差し出す(滝本=要件を運ぶ人「電気が戻った日に、一斉に上げるがです」、戸倉=台帳の読み合わせ+**"何か"のストレート**(美咲にだけ・一度だけ)、狩野=「どれを最後まで止めないか」の完成、守屋=人の導線、城戸=備えの費用の即決裁、風間=非常時権限の点検、矢吹=復元順の読み上げ)。誰も万能にしない・入退場丁寧はここでも厳守。
- 美咲のアルバム最終回収(裏書き=祖母の字)。新人一人が初登場(名前と問いの受け渡しのみ)——最終場面で美咲が「それは、本当に戻せるんですか」を次の世代へ(円環の完成)。
...
| 新人(名前のみ・継承の受け皿) | **S10で初登場** | S10最終場面で美咲が第一の問いを渡す(円環)。掘り下げない |
