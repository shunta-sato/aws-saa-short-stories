...
試験は「シナリオを読んで最適なサービスを選ぶ」形式です。用語を孤立した定義で暗記するより、**誰が・どんな要件で・なぜそれを選び・なぜ他を捨てたか**という文脈ごと記憶する方が、本番の判断力に直結します。1編はおおむね15〜20分で読める長さで、物語の山場はすべて技術選定の分岐に置かれています。
...
全7話。シーズンアーク「取り出せる形」——社是(誰かがあとで必要とする記録を、取り出せる形で残す)が、請求書・障害・移行・訓練を通じて実証されるシーズン。
...
| 幕間 | [借りない倉庫](stories/data_ingestion_and_analytics/ex02_借りない倉庫.md) | Redshift Serverless——RPU従量の借り方(用語集外の頻出補強) |
